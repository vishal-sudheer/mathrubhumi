...
export default function CategoriesMaster() {
  ...
  const handleAddCategory = async () => {
    ...
    try {
      await api.post('/auth/category-create/', payload);
      setModal({
        isOpen: true,
      ...
      if (page === 1) {
        fetchAllCategories({ page: 1, pageSize });
      } else {
        setPage(1);
      }
      setFormData({
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            try {
              ...
              await fetchAllCategories({ page, pageSize });
              setModal({
                isOpen: true,
  ...
  const fetchAllCategories = async (options = {}) => {
                ...
                {items.length === 0 ? (
                  <tr>
                    <td colSpan="2" className="px-4 py-8 text-center text-gray-400">
                      {isLoading ? 'Loading categories...' : 'No categories found. Add one below.'}
                    </td>
                  </tr>
                ) : (
